...
router.post("/auth/signup", async (req, res) => {
  ...
  console.log(`[AUTH] Signup: ${email}. IP: ${ip}. Fraud Score: ${fraudScore}`);
...
router.post("/auth/login", async (req, res) => {
  ...
  if (!formData || !behavioralData) {
    return res
      .status(400)
      .json({ error: "Invalid request structure from login form." });
  }
  ...
  console.log(`[AUTH] Login: ${email}. IP: ${ip}. Fraud Score: ${fraudScore}`);
  res
    .status(200)
    .json({
