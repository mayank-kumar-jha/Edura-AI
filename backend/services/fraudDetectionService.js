...
try {
  ...
  if (fs.existsSync(modelPath) && fs.existsSync(scalerPath)) {
    ...
    console.error(
      'CRITICAL: Model or scaler file not found. Please train the model by running "node ml/trainModel.js"'
    );
    process.exit(1);
  }
} catch (error) {
...
function predictFraud(features) {
  ...
  const scaledFeatures = features.map((value, index) => {
    ...
    // Clamp the value between 0 and 1 to handle edge cases where input is outside training range
    return Math.max(0, Math.min(1, scaledValue));
  });

  console.log("[DEBUG] Raw features:", features);
